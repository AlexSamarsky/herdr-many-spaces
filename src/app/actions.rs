...
impl AppState {
    ...
    pub(crate) fn workspace_close_indices(&self, ws_idx: usize) -> Vec<usize> {
        // Close what the sidebar draws as one group, which is the space that
        // owns the rows and the rows themselves - not every space that happens
        // to stand on the same repository. Grouping by repository key would take
        // a second agent's space down with the first agent's rows.
        self.workspaces
            .get(ws_idx)
