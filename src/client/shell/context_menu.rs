...
impl ClientShellState {
    pub(super) fn open_workspace_context_menu(&mut self, workspace_id: String, x: u16, y: u16) {
        ...
        // The menu talks about the group the sidebar draws, so it asks the same
        // authority the sidebar groups by rather than counting spaces on the
        // repository.
        let index = snapshot
            .workspaces
            .iter()
            .position(|candidate| candidate.workspace_id == workspace_id);
        let group_key = index.and_then(|index| super::render::workspace_group_key(snapshot, index));
        let has_worktree_children = worktree.is_some_and(|worktree| !worktree.is_linked_worktree)
            && group_key.as_deref().is_some_and(|key| {
                (0..snapshot.workspaces.len())
                    .filter(|candidate| {
                        super::render::workspace_group_key(snapshot, *candidate).as_deref()
                            == Some(key)
                    })
                    .count()
                    >= 2
            ...
            ClientContextMenuAction::ToggleGroup => {
                let key = self.snapshot.as_deref().and_then(|snapshot| {
                    snapshot
                        ...
                        .and_then(|index| super::render::workspace_group_key(snapshot, index))
                });
                if let Some(key) = key {
