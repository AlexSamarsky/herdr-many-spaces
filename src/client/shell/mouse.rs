...
impl ClientShellState {
    ...
    fn workspace_move_method(
        ...
        // Dragging a group's head moves the block the sidebar draws under it -
        // its own rows, and not another agent's space that merely stands on the
        // same repository.
        let source_group = snapshot
            .workspaces
            .iter()
            .position(|workspace| workspace.workspace_id == source.workspace_id)
            .and_then(|index| super::render::workspace_group_key(snapshot, index));
        if let Some(group_key) = source_group {
            let workspace_ids = std::iter::once(source.workspace_id.clone())
                .chain(
                    (0..snapshot.workspaces.len())
                        .filter(|index| {
                            snapshot.workspaces[*index].workspace_id != source.workspace_id
                        })
                        .filter(|index| {
                            super::render::workspace_group_key(snapshot, *index).as_deref()
