...
impl ClientShellState {
    ...
    pub(super) fn open_confirm_close_overlay(&mut self, workspace_id: String) {
        ...
        // Closing a group closes what the sidebar shows inside it: the owning
        // space and its rows.
        let group_key = workspace
            ...
            .and_then(|_| {
                snapshot
                    .workspaces
                    .iter()
                    .position(|member| member.workspace_id == workspace.workspace_id)
        ...
        let group = group_key
            ...
            .map(|key| {
                (0..snapshot.workspaces.len())
                    .filter(|index| {
                        super::render::workspace_group_key(snapshot, *index).as_deref() == Some(key)
                    })
