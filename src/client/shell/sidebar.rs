...
/// The group a workspace is drawn in, and the single authority for it.
///
/// A row belongs to the space that opened it, which is what keeps two agents on
/// one repository apart: grouping by repository key alone would draw the second
/// agent's own space inside the first agent's group, along with both agents'
/// rows. A space that owns rows heads its own group and answers with its own id.
///
/// Two rows fall back to the repository key: one opened before the owner field
/// existed, and one whose owner has since been closed. They then group the way
/// they always did rather than scattering.
pub(crate) fn workspace_group_key(snapshot: &ClientShellSnapshot, index: usize) -> Option<String> {
    let workspace = snapshot.workspaces.get(index)?;
    let worktree = workspace.worktree.as_ref()?;
    if !worktree.is_linked_worktree {
        return Some(workspace.workspace_id.clone());
    }
    if let Some(owner) = worktree.parent_workspace_id.as_deref() {
    ...
    snapshot
        ...
        .find(|candidate| {
            candidate
                .worktree
                .as_ref()
                .is_some_and(|candidate| candidate.key == worktree.key && !candidate.is_linked_worktree)
        })
        .map(|candidate| candidate.workspace_id.clone())
        .or_else(|| Some(worktree.key.clone()))
}
...
pub(crate) fn workspace_entries(
    ...
    let keys = (0..snapshot.workspaces.len())
        .map(|index| workspace_group_key(snapshot, index))
        .collect::<Vec<_>>();
    ...
    for index in 0..snapshot.workspaces.len() {
        ...
        if !emitted.insert(group_key) {
            continue;
        }
        let Some(group_members) = members.get(group_key) else {
            continue;
        };
        let parent = group_members
            .iter()
            .copied()
            .find(|member| snapshot.workspaces[*member].workspace_id == group_key)
            .or_else(|| {
        ...
        if collapsed_groups.contains(group_key) {
...
fn parent_group_key(snapshot: &ClientShellSnapshot, index: usize) -> Option<String> {
    ...
    let key = workspace_group_key(snapshot, index)?;
    ((0..snapshot.workspaces.len())
        .filter(|candidate| workspace_group_key(snapshot, *candidate).as_deref() == Some(key.as_str()))
        .count()
        >= 2)
        .then_some(key)
}
...
fn displayed_workspace_status(
    ...
    // Only a group's head folds its members' statuses into itself; a row reports
    // its own.
    if !workspace
        .worktree
        .as_ref()
    ...
    let Some(group_key) = snapshot
        .workspaces
        .iter()
        .position(|candidate| candidate.workspace_id == workspace.workspace_id)
        .and_then(|index| workspace_group_key(snapshot, index))
    else {
        return workspace.agent_status;
    };
    if !collapsed_groups.contains(&group_key) {
    ...
    // A folded group still has to report what its rows are doing, so the busiest
    // status among the members wins.
    (0..snapshot.workspaces.len())
        .filter(|index| workspace_group_key(snapshot, *index).as_deref() == Some(group_key.as_str()))
