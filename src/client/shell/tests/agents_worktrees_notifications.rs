...
fn space(id: &str, label: &str, cwd: &str, worktree: Option<ClientShellWorktree>) -> ClientShellWorkspace {
    ClientShellWorkspace {
        workspace_id: id.into(),
        active_tab_id: format!("tab_{id}"),
        new_workspace_cwd: cwd.into(),
        number: 1,
        label: label.into(),
        custom_label: false,
        branch: None,
...
fn membership(is_linked: bool, owner: Option<&str>) -> Option<ClientShellWorktree> {
    Some(ClientShellWorktree {
        key: "repo".into(),
        label: "repo".into(),
        is_linked_worktree: is_linked,
        parent_workspace_id: owner.map(str::to_owned),
    })
}

#[test]
fn rows_group_under_the_space_that_opened_them() {
    // Two agents, one repository, one row each. Grouping by repository key alone
    // put every row - and the second agent's own space - inside the first
    // agent's group, which is the whole reason the owner is recorded.
    let mut snapshot = snapshot();
    snapshot.workspaces[0].worktree = membership(false, None);
    snapshot.workspaces.push(space("ws_2", "agent-two", "/repo", membership(false, None)));
    snapshot.workspaces.push(space("ws_3", "one", "/repo/one", membership(true, Some("ws_1"))));
    snapshot.workspaces.push(space("ws_4", "two", "/repo/two", membership(true, Some("ws_2"))));

    let entries = crate::client::shell::render::workspace_entries(&snapshot, &HashSet::new());
    let drawn = entries
        .iter()
        .map(|entry| (snapshot.workspaces[entry.index].workspace_id.as_str(), entry.indented))
        .collect::<Vec<_>>();

    assert_eq!(
        drawn,
        vec![("ws_1", false), ("ws_3", true), ("ws_2", false), ("ws_4", true)]
    );
}

#[test]
fn a_row_without_an_owner_falls_back_to_the_repository_parent() {
    // A row opened before the owner existed, or one whose owner has been closed,
    // groups the way it always did rather than standing alone at the top level.
    let mut snapshot = snapshot();
    snapshot.workspaces[0].worktree = membership(false, None);
    snapshot.workspaces.push(space("ws_2", "legacy", "/repo/legacy", membership(true, None)));
    ...
    assert_eq!(drawn, vec![("ws_1", false), ("ws_2", true)]);
}
...
fn grouped_worktrees_render_parent_branch_and_indented_child() {
    ...
    // key comes from the authority the sidebar groups by.
    state.collapsed_groups.insert(
        crate::client::shell::render::workspace_group_key(
