...
pub struct ClientShellWorktree {
    ...
    /// The space this row was opened from. The sidebar groups by it, so that two
    /// spaces on one repository keep their own rows instead of collapsing into
    /// whichever of them the server happened to list first. Absent for a space
    /// that owns rows rather than being one, and for a row recorded before the
    /// field existed.
